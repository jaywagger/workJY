...
/*
 *  <<상속관계에서 생성자가 갖는 특징>>
 *  1. 클래스의 모든 생성자메소드의 첫 번째 문장에는 부모클래스의 기본생성자를
 *     호출하는 명령문이 생략되어 있다.
...
class SubA extends SuperA{
	...
	int point;
	SubA(){
		super();   //모든 생성자메소드의 첫 번째 문장에는 super()호출
	}
	SubA(String addr){
		super();
		this.addr = addr;
...
public class InheritanceTest03 {
	public static void main(String[] args) {
		...
		/*System.out.println(obj.name+","+obj.addr+","
				+obj.age+","+obj.point);*/ //private 때메. 직접 노
		
		System.out.println(obj.getName()+","+obj.addr+","
				+obj.getAge()+","+obj.point); //간접 오케이
	}
